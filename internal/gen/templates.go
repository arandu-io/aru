...
// serves all three. Anything a key or an index names is VARCHAR rather than TEXT,
// because MySQL refuses TEXT in a key without a prefix length. The id is
// generated by the application: gen_random_uuid, UUID() and randomblob are three
...
func (s *Service) List(ctx context.Context, actor security.Subject, q data.Query) ([]{{.Entity}}, error) {
	g, err := security.Authorize(ctx, s.policy, actor, ActionList, {{.Entity}}{})
	if err != nil {
		return nil, err
	}
